...
<form action="${pageContext.servletContext.contextPath }/find_pass.do"
      class="form-signin" method="post" id="findpwybtn">
      <h1 align="center" class="h3 mb-3 font-weight-normal">재미나</h1>
      <h3 align="center">비밀번호 찾기</h3>
      <label for="inputId" class="sr-only">아이디</label> <input type="txt"
         id="inputId" class="form-control" name="getId" onkeyup="getid(this);"
         placeholder="아이디" required autofocus> <small id="checked3"></small>

      <label for="inputId" class="sr-only">이메일주소</label> <input type="email"
         id="inputId" class="form-control" name="email"
         onkeyup="getemail2(this)" placeholder="이메일" required autofocus>
      <small id="checked2"></small>
      <button class="btn btn-lg btn-primary btn-block" type="sumit"
         id="findpw" disabled="disabled">비밀번호 찾기</button>
         <a href="${pageContext.servletContext.contextPath }/login.do">로그인</a><br/>
         <a href="${pageContext.servletContext.contextPath }/find_user.do">아이디 찾기</a>
